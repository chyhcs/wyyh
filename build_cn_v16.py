# -*- coding: utf-8 -*-
"""
讯飞 × RNP 技术交流 PPT v16：11 页主汇报 + 问答目录页 + 13 页附录，基于华为简报模板。
相对 v11（对齐华为 × 讯飞联合人才培养方案 RNP_AI_Talent_20260909）：
  ...
  · v14：第 4、5、6 三页把建设步骤写实，每一步都写明讯飞做什么、RNP 做什么，取消与步骤重复的底部两框。
    ...
    v16：主汇报全面要点化——列表改自动编号（1. 2. 3.），破题句里的并列项改行内圈码（① ② ③）。
    卡片沿用原有的 1/2/3 编号，与行内圈码区分。事实口径不变。
...
用法：python3 build_cn_v16.py <template.pptx> <out.pptx>
"""
import sys
...
OUT = sys.argv[2] if len(sys.argv) > 2 else "RNP-讯飞交流-v16.pptx"
RNP_C = "1F4E79"
...
thesis(s, "讯飞交付　① 平台软件及授权　② 部署配置　③ API 接入说明　④ 验收报告　⑤ 运维文档；验收后由 RNP 管理，讯飞现场陪伴 12 个月")
...
thesis(s, "交付 ① 文本大模型（≥290B 参数）　② 语音识别　③ 语音合成，各 1 个；随附权重、模型卡、评测报告与部署说明", h=0.55)
...
thesis(s, "交付 ① 国民 APP（多终端）　② 数据运营平台，面向 RNP 批准的授权用户；问答 1,000 路并发、语音识别与合成各 200 路，分别实测", h=0.52)
...
thesis(s, "四个阶段每轮共 608 课时，实施两个轮次；移交后 RNP 能独立完成　① 平台运维　② 训练提交　③ 模型部署　④ 版本发布")
legend(s, 1.76)
phases = [
...
thesis(s, "验收依据　① SOW　② BOQ　③ 功能清单　④ 批准方案；新增需求按书面变更处理。会中记录结论，会后形成双方行动清单")
gap = 0.3
cw = (CW - 2 * gap) / 3
cy, ch = 1.85, 1.5
cols = [
