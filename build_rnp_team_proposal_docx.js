...
// =====================================================================================
const GROUPS = [
  { name: "项目与产品", intro: "负责项目在 RNP 侧的组织、进度、资源与验收，以及国民 AI 应用的产品定义。建议第一批到位。", roles: [
  ...
  { name: "语料与数据资源", intro: "组织葡语语料的获取、授权、生产与质检，是第一批到位的主要力量。", roles: [
    ...
    { name: "标注员", en: "Annotator", c: 0, e: 20, one: "按规范完成文本标注、安全样本与语音转写",
      req: ["葡语母语，书面表达规范，细致稳定", "经培训后能按标注规范作业，转写岗位需良好的听写能力", "可由高校学生或合同人员承担，建议保留固定骨干"],
      col: ["使用双方确认的标注规范、样例与工具作业，配合科大讯飞在约定批次、抽样范围和复验轮次内开展质量复核"] },
  ] },
  { name: "葡语语言专家", intro: "定义巴西葡语的质量、标注与语音标准。建议核心 1 人牵头，其余专家可来自高校。", roles: [
  ...
  { name: "业务与领域专家", intro: "为三个领域大模型确定业务场景并评估领域结果。建议第二批到位。", roles: [
    ...
    { name: "领域专家", en: "Domain Expert", c: 0, e: 2, one: "在领域内评价模型输出结果",
      req: ["领域内资深专业人员，可为兼职或来自高校、机构的短期专家", "能判断模型输出的正确性与适用性"],
      col: ["参加 RNP 组织的领域评测，出具专业评价意见，配合科大讯飞开展评测结果分析"] },
  ] },
  { name: "模型与测试工程", intro: "在模型训练中心通过技术交流和约定的实操参与国家模型的训练与评测，并承担 RNP 负责的领域模型训练与评测；国家模型训练的合同责任分工不变。建议第二批到位，是知识转移的核心受训对象。", roles: [
  ...
  { name: "应用与运营", intro: "承担国民 AI 应用与 RNP 系统的集成、用户审批、内容与日常运营，并维护已交付且授权修改的代码。建议第三批到位。", roles: [
  ...
  { name: "平台运维", intro: "验收后运行国家 AI 能力中心平台。建议第三批到位，全部为核心团队。华为数据中心层面的运维人员不在本建议范围内。", roles: [
...
P("本建议依据 9 月 9 日技术交流的内容和当前的工作范围划分测算，供 RNP 规划团队时参考。岗位的具体设置、人员来源、到位时间和经费安排，由 RNP 根据自身情况确定。科大讯飞愿在 RNP 形成团队方案后共同评审，并据此调整知识转移的安排。", { after: 60 });
...
P(`**核心团队（Equipe núcleo）${CORE} 人。** 建议由 RNP 自有人员组成，贯穿项目全程，是知识转移的主要对象，项目结束后转为常设运营团队。核心团队包括三类岗位：对项目成果负责和签署验收的岗位；项目结束后独立运行平台、模型与应用的岗位；承接知识转移并向其他人员传授的技术骨干。`);
P(`**扩展团队（Equipe complementar）${EXT} 人。** 承担语料生产、领域与语言专业支持、应用本地化开发与运营等工作量集中的任务，按工作包投入和退出。人员可通过科研津贴、固定期限合同或合作机构参与，具体方式由 RNP 按自身的人事与经费规则确定。建议每一类扩展团队岗位对应一个核心团队岗位作为负责人，以保证工作的连续性和成果的沉淀。`);
P("人数为按工作内容测算的建议配置。同一人可以承担多个岗位的工作，RNP 可结合现有人员和各阶段的实际工作量调整。", { after: 60 });
