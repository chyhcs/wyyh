...
for i, (name, desc, when) in enumerate(steps):
    ...
    dot(s, bx + (bw - 0.46) / 2, fy + 0.22, 0.46, str(i + 1), size=13)
    text(s, bx + 0.1, fy + 0.78, bw - 0.2, 0.42, name, size=13.5, color=TXT, bold=True, align="c", valign="m", margin=(0, 0, 0, 0), sa=0)
    body(s, bx + 0.2, fy + 1.28, bw - 0.4, 0.8, desc, size=11.5, align="c")
    text(s, bx + 0.2, fy + 2.12, bw - 0.4, 0.38, when, size=12, color=BODY, bold=True, align="c", valign="m", margin=(0, 0, 0, 0), sa=0)
    if i < 4:
        arrow(s, bx + bw + 0.03, fy + 0.9)
band(s, 3.95, 0.58, "华为提供", "训练与推理算力　·　部署环境与网络　·　数据中心运维衔接　　（三方责任中的华为部分）", title_w=1.6)
band(s, 4.7, 0.58, "知识转移", "贯穿五个阶段：巴方人员在每个阶段与讯飞一起做，不是等交付完成后才开始学", fill=REDT, line=None, title_w=1.6)
band(s, 5.45, 0.58, "关键里程碑", "{{示例：M2 语料到位　·　M5 底座通过评测　·　M8 应用联调通过　·　M10 验收　·　M12 交接完成}}", title_w=1.6)
note(s, 6.2, "阶段名称、数量与时间按实际方案确定。RNP 据此编制本方项目计划。")
notes(s, "回答“这些工作怎样安排在一起”。Mota 原话是三方责任，华为平台依赖单列，避免 RNP 把算力、环境的事算到讯飞头上。")

# ============================== 7 RNP 投入什么 ==============================
s = slide("RNP 需要投入什么：人员与资源")
rows = [
    ["项目负责人", "统筹巴方资源，对接讯飞与华为", "项目管理，英语沟通", "全部 · 阶段 1–5", "{{示例：1}}"],
    ["领域专家", "定义业务问题，评价领域成果", "领域业务经验", "领域模型 · {{示例：阶段 3–4}}", "{{示例：每领域 2}}"],
    ["数据工程师", "语料收集、清洗、标注", "Python，数据处理基础", "模型、领域模型 · {{示例：阶段 1–2}}", "{{示例：3}}"],
    ["模型工程师", "参与训练、评测与调优", "Python，机器学习基础", "模型、领域模型 · {{示例：阶段 2–3}}", "{{示例：3}}"],
    ["应用开发", "参与应用开发与系统对接", "软件开发经验", "APP · {{示例：阶段 3–4}}", "{{示例：2}}"],
    ["运维工程师", "部署、运行与监控", "Linux 与容器基础", "全部 · {{示例：阶段 4–5}}", "{{示例：2}}"],
]
table(s, L, 1.2, [1.8, 3.4, 2.6, 3.0, 1.53], ["角色", "做什么", "基础要求", "参与哪项 · 阶段", "人数"], rows, row_h=0.5, header_h=0.45, size=11.5)
band(s, 5.15, 0.75, "资源", "数据与授权　·　环境与访问　·　业务系统接口　·　测试条件　　到位时间：{{示例：合同生效后 4 周内}}", title_w=1.1)
note(s, 6.15, "RNP 据此编制本方参与计划；角色按实际任务取舍。这些人也是下一页知识转移的对象。")
...
body(s, L + 0.3, cy + 1.5, cw - 0.6, ch - 1.7, [
    "方式：讲解、示范、共同操作、指导实践",
    "对象：上一页的参与人员，名额 {{示例：每阶段 10 人}}",
    "结果：数据工程师能独立构建数据集；模型工程师能独立微调与评测；应用开发能独立开发部署；运维工程师能独立运行",
    "语言：{{示例：英语授课，配葡语助教}}",
...
band(s, 5.45, 0.6, "整体中的位置", "本页是华为能力建设计划（{{示例：2900 人}}）中讯飞承担的部分，由华为统一汇总为一份 program；{{口径待核实}}", title_w=1.8)
note(s, 6.25, "不覆盖：讯飞 SOW 以外的工作包。“已包含”口径以 SOW 与报价核对为准。")
