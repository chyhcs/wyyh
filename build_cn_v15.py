# -*- coding: utf-8 -*-
"""
讯飞 × RNP 技术交流 PPT v15：11 页主汇报 + 问答目录页 + 13 页附录，基于华为简报模板。
相对 v11（对齐华为 × 讯飞联合人才培养方案 RNP_AI_Talent_20260909）：
  ...
  · v14：第 4、5、6 三页把建设步骤写实，每一步都写明讯飞做什么、RNP 做什么，取消与步骤重复的底部两框。
    ...
    事实取自 A2/A3/A4/A5 与 SOW，长限定语留在附录。
...
用法：python3 build_cn_v15.py <template.pptx> <out.pptx>
"""
import sys
...
OUT = sys.argv[2] if len(sys.argv) > 2 else "RNP-讯飞交流-v15.pptx"
RNP_C = "1F4E79"
    ...
    for i, (name, xf_t, rn_t, out_t) in enumerate(items):
        ...
        for k, (who, col, fl, t) in enumerate([("讯飞", RED, XF, xf_t), ("RNP", RNP_C, RN, rn_t)]):
            ...
            text(s, bx + 0.28, by + 0.03, 1.4, 0.22, who, size=9.5, color=col, bold=True,
                 valign="m", margin=(0, 0, 0, 0), sa=0)
            body(s, bx + 0.28, by + 0.26, bw - 0.56, blk - 0.30, t, size=size, ls=1.3)
...
thesis(s, "文本大模型（≥290B 参数）、语音识别、语音合成各 1 个；权重、模型卡、评测报告与部署说明交付 RNP", h=0.55)
...
step_cards(s, 2.06, [
    ("1  语料与基座准备",
     "中国境内自有算力采集公开葡语语料，增强基座的通用葡语能力；语料一并提交巴西方",
     "巴西生产语料并取得授权，约 500 亿 Token；{{协调数据中心建成前的算力}}",
     "增强后的基座 + 本地语料库"),
    ("2  模型训练",
     "在增强后的基座上继续训练，产出 NLP、ASR、TTS 三个模型",
     "按讯飞规范持续供料，配合抽样质检",
     "NLP ≥290B、ASR、TTS 各 1 个"),
    ("3  评测",
     "出具评测报告：回归、鲁棒性、偏见、安全与文化适配",
     "共同确定测试集与通过规则；葡语与领域专家评审",
     "评测报告，双方签认"),
    ("4  本地交付与持续优化",
     "巴西环境部署，交付权重与模型卡；12 个月内 NLP 升级 3 次、语音各 1 次",
     "组织验收与发布；按 A2 的上限提供升级语料",
     "权重、版本清单、模型卡、部署说明"),
])
means(s, 5.56, "基座增强先行，不占用巴西算力，也不必等本地语料齐备才开始；本地语料和算力到位即可进入训练，工期风险更小", h=0.6)

# ============================== 5 国家 AI 应用 ==============================
s = slide("国家 AI 应用：讯飞交付软件，RNP 负责内容、用户与运营", size=21, ref="A4")
thesis(s, "国民 APP 与数据运营平台，面向 RNP 批准的授权用户；问答 1,000 路并发、语音识别与合成各 200 路，分别实测", h=0.52)
card(s, L, 1.62, CW, 1.14, fill=PANEL)
label(s, L + 0.26, 1.68, 4.0, "产品是什么", color=TXT, size=12.5)
for i, t in enumerate(["手机 APP", "电脑端", "数据运营平台"]):
    chip(s, L + 2.10 + i * 2.30, 1.70, 2.16, 0.30, t, fill=WHITE, color=TXT, size=10.5, line=LINE)
funcs = ["问答", "检索", "文档分析", "研究辅助", "语音输入与朗读", "笔记与写作"]
fw6 = (CW - 0.52 - 5 * 0.12) / 6
for i, t in enumerate(funcs):
    fx = L + 0.26 + i * (fw6 + 0.12)
    card(s, fx, 2.34, fw6, 0.34, fill=WHITE, line=LINE)
    text(s, fx, 2.34, fw6, 0.34, t, size=10.5, color=TXT, bold=True, align="c", valign="m", margin=(0, 0, 0, 0), sa=0)
step_cards(s, 2.94, [
    ("1  明确需求",
     "提供标准产品能力清单与可配置项",
    ...
    ("2  本地化",
     ...
     "提供品牌素材与栏目内容；{{葡语 PPT 与公文模板待适配验证}}",
     "按指定浏览器与操作系统验证通过"),
    ("3  系统集成",
     "完成接口对接与联调，提供开发者文档与示例",
     "提供本地系统接口与账号凭证；落实第三方 API 的额度与许可",
     "HTTP/REST/JSON/WebSocket 接口联调通过"),
    ("4  上线运营",
     "Stage 1 初步交付，Stage 2 扩展运行；移交运维文档",
     "审批用户；产品、运营与审核团队承接并自主演进",
     "APP 与数据运营平台上线"),
], h=2.60)
means(s, 5.84, "产品团队在需求阶段即需到位，运营团队在 Stage 1 交付前到位；交付版本可由 RNP 自主演进", h=0.6)

# ============================== 6 DLM ==============================
s = slide("领域模型 DLM：RNP 训练，讯飞指导；3 项服务，每项 1 个场景", size=21, ref="A5")
thesis(s, "四个步骤都由 RNP 执行、讯飞在旁指导——这与国家模型相反，是 RNP 第一次自己训练模型", h=0.5)
band(s, 1.60, 0.48, "服务范围", "3 项行业模型辅助训练服务，每项 1 个边界清晰的固定场景；领域由 RNP 提出、讯飞评估可行性；模型权重由 RNP 训练产生并归 RNP", size=11)
step_cards(s, 2.26, [
    ("1  确定场景",
     "评估领域可行性并出技术方案；核实基座版本、规模与许可",
     "提出领域与场景，明确用户、输入输出和边界",
     "签署《DLM 场景及技术方案确认书》"),
    ("2  准备数据与路线",
     "给出路线建议（LoRA-SFT、SFT、GRPO 等），每项确认 1 条主要路线",
     "提供训练、验证、安全与独立评测数据及环境",
     "数据与训练路线书面确认"),
    ("3  训练与评测",
    ...
    ("4  服务验收",
     ...
     "核对指导记录，完成业务验收",
     "服务验收，与模型效果目标分开"),
], out_label="完成依据", h=3.05)
note(s, 5.54, "生产部署、API 开发与应用集成不在本服务内，可另行约定。", h=0.28, size=9.5)
