# -*- coding: utf-8 -*-
"""
讯飞 × RNP 技术交流 PPT v7：11 页主汇报（新版式）+ 分隔页 + 11 页附录，基于华为简报模板。
版式规则（按三页样张确认）：
...
用法：python3 build_cn_v7.py <template.pptx> <out.pptx>
"""
import sys
...
OUT = sys.argv[2] if len(sys.argv) > 2 else "RNP-讯飞交流-v7.pptx"
RNP_C = "1F4E79"
...
def slide(title, size=22, ref=None):
    ...
    if ref:
        w = 1.6 + 0.35 * ref.count("·")
        chip(s, R - w, 0.32, w, 0.3, "详见附录 " + ref, fill=WHITE, color=MUTE, size=9.5, line=LINE)
    return s
...
body(s, L + 0.25, 4.62, CW - 0.5, 0.58, "**知识转移**　四阶段共 578 小时、约 74 个工作日，RNP 核心团队从跟岗到独立操作　　**交付后支持**　现场陪伴 12 个月 + 远程支持 60 个月", size=12, valign="m")
body(s, L, 5.4, CW, 0.7, [
...
for i, (name, hours, meta, tasks, fill) in enumerate(phases):
    ...
    text(s, px + 0.2, py + 0.5, pw - 0.4, 0.46, [f"**{hours}** 小时"], size=12, color=TXT, valign="m", margin=(0, 0, 0, 0), sa=0)
