# -*- coding: utf-8 -*-
"""
讯飞 × RNP 技术交流 PPT v14：11 页主汇报 + 问答目录页 + 13 页附录，基于华为简报模板。
相对 v11（对齐华为 × 讯飞联合人才培养方案 RNP_AI_Talent_20260909）：
  ...
  · v14：第 4、5、6 三页把建设步骤写实，每一步都写明讯飞做什么、RNP 做什么，取消与步骤重复的底部两框。
    三页形状各不相同：第 4 页步骤为列、讯飞/RNP 为行（转置矩阵）；第 5 页产品横带 + 三列步骤表；
    第 6 页四列步骤表，多一列「完成依据」。事实取自 A2/A3/A4/A5 与 SOW。
...
用法：python3 build_cn_v14.py <template.pptx> <out.pptx>
"""
import sys
...
OUT = sys.argv[2] if len(sys.argv) > 2 else "RNP-讯飞交流-v14.pptx"
RNP_C = "1F4E79"
...
thesis(s, "文本大模型（≥290B 参数）、语音识别、语音合成各 1 个；权重、版本清单、模型卡、评测报告和部署说明交付 RNP", h=0.55)
text(s, L, 1.66, CW, 0.28, "第 1 步两条线同时进行：讯飞在中国境内先行增强基座，RNP 在巴西建设本地语料；本地语料就绪、算力到位后进入第 2 步",
     size=10.5, color=MUTE, valign="m", margin=(0, 0, 0, 0), sa=0)
gw, gg = 1.0, 0.12
sw = (R - L - gw - gg - 3 * 0.2) / 4
sx = [L + gw + gg + i * (sw + 0.2) for i in range(4)]
steps4 = ["1  语料与基座准备", "2  模型训练", "3  评测", "4  本地交付与持续优化"]
for i, nm in enumerate(steps4):
    card(s, sx[i], 2.02, sw, 0.34, fill=PANEL2, line=LINE)
    text(s, sx[i], 2.02, sw, 0.34, nm, size=11.5, color=TXT, bold=True, align="c", valign="m", margin=(0, 0, 0, 0), sa=0)
grid4 = [
    ("讯飞负责", RED, XF, 2.42, [
        "在中国境内投入自有算力，采集并处理公开葡语语料，继续训练讯飞基础模型，提升其通用葡语能力；采集到的公开语料一并提交巴西方",
        "在已具备通用葡语能力的基础模型上继续训练，产出 NLP、ASR、TTS 三个模型；按批准的模型技术方案与里程碑执行",
        "出具评测报告，覆盖回归、鲁棒性、偏见、安全、幻觉与文化适配",
        "在巴西环境部署，交付权重、版本清单、模型卡与部署说明；交付后 12 个月内 NLP 升级 3 次、ASR 与 TTS 各 1 次",
    ]),
    ("RNP 怎么参与", RNP_C, RN, 4.00, [
        "在巴西生产葡语语料并取得授权，约 500 亿 Token；{{协调数据中心建成前用于语料采集与处理的算力}}",
        "按讯飞的规范、工具与试批要求持续供料并配合抽样质检；技术路线、次数与初始交付时间实施前书面确认",
        "与讯飞共同确定测试集与通过规则；组织葡语与领域专家参与评审",
        "组织验收与发布；持续优化期内按 A2 的数据上限提供升级语料",
    ]),
]
for nm, col, fl, gy, cells in grid4:
    card(s, L, gy, gw, 1.50, fill=WHITE, line=col)
    text(s, L, gy, gw, 1.50, nm, size=11.5, color=col, bold=True, align="c", valign="m", margin=(0.04, 0, 0.04, 0), sa=0)
    for i, t in enumerate(cells):
        card(s, sx[i], gy, sw, 1.50, fill=fl)
        body(s, sx[i] + 0.16, gy + 0.10, sw - 0.32, 1.30, t, size=10.5, ls=1.3)
means(s, 5.72, "基座增强先行，不占用巴西算力，也不必等本地语料齐备才开始；本地语料和算力到位即可进入国家模型训练，工期风险更小", h=0.62)

# ============================== 5 国家 AI 应用 ==============================
s = slide("国家 AI 应用：讯飞交付软件，RNP 负责内容、用户与运营", size=21, ref="A4")
thesis(s, "国民 APP 与数据运营平台，面向 RNP 批准的授权用户；问答 1,000 路并发、语音识别与合成各 200 路，按服务分别配置并实测", h=0.55)
card(s, L, 1.66, CW, 1.20, fill=PANEL)
label(s, L + 0.26, 1.74, 4.0, "产品是什么", color=TXT, size=12.5)
for i, t in enumerate(["手机 APP", "电脑端", "数据运营平台"]):
    chip(s, L + 2.10 + i * 2.30, 1.76, 2.16, 0.30, t, fill=WHITE, color=TXT, size=10.5, line=LINE)
funcs = ["问答", "检索", "文档分析", "研究辅助", "语音输入与朗读", "笔记与写作"]
fw6 = (CW - 0.52 - 5 * 0.12) / 6
for i, t in enumerate(funcs):
    fx = L + 0.26 + i * (fw6 + 0.12)
    card(s, fx, 2.44, fw6, 0.34, fill=WHITE, line=LINE)
    text(s, fx, 2.44, fw6, 0.34, t, size=10.5, color=TXT, bold=True, align="c", valign="m", margin=(0, 0, 0, 0), sa=0)
c5 = [(L, 2.20), (2.85, 4.90), (7.90, 4.93)]
for (cx, cwd), nm, col in zip(c5, ["怎么落地 · 四步", "讯飞负责", "RNP 怎么参与"], [MUTE, RED, RNP_C]):
    text(s, cx + (0.0 if nm.startswith("怎么") else 0.18), 2.96, min(cwd, 2.6), 0.28, nm,
         size=11.5 if nm.startswith("怎么") else 12, color=col, bold=True, valign="m", margin=(0, 0, 0, 0), sa=0)
rows5 = [
    ("1  明确需求", "提供标准产品能力清单与可配置项，输出需求与功能设计文档", "参与产品定义与功能设计；确定用户范围、内容规则与品牌"),
    ("2  本地化", "葡语界面与本地化配置，接入 RNP 品牌与内容；按指定浏览器与操作系统验证", "提供品牌素材、栏目内容；{{葡语 PPT 生成与本地公文模板待适配验证}}"),
    ("3  系统集成", "完成接口对接与联调；提供开发者文档与示例（HTTP/REST/JSON/WebSocket）", "提供本地系统接口与账号凭证；落实搜索等第三方 API 的额度、许可与费用"),
    ("4  上线运营", "Stage 1 初步交付，Stage 2 扩展运行；配置平台并移交运维文档", "审批用户；由产品、运营与审核团队承接，可在交付版本上自主演进"),
]
for i, (st, xf5, rn5) in enumerate(rows5):
    ry = 3.28 + i * 0.70
    card(s, L, ry, 2.20, 0.64, fill=PANEL2, line=LINE)
    text(s, L + 0.18, ry, 2.0, 0.64, st, size=11.5, color=TXT, bold=True, valign="m", margin=(0, 0, 0, 0), sa=0)
    card(s, 2.85, ry, 4.90, 0.64, fill=XF)
    body(s, 3.03, ry + 0.05, 4.54, 0.54, xf5, size=10.5, valign="m", ls=1.25)
    card(s, 7.90, ry, 4.93, 0.64, fill=RN)
    body(s, 8.08, ry + 0.05, 4.57, 0.54, rn5, size=10.5, valign="m", ls=1.25)
means(s, 6.18, "产品团队在需求阶段即需到位，运营团队在 Stage 1 交付前到位；交付版本可由 RNP 自主演进", h=0.6)

# ============================== 6 DLM ==============================
s = slide("领域模型 DLM：RNP 训练，讯飞指导；3 项服务，每项 1 个场景", size=21, ref="A5")
thesis(s, "四个步骤都由 RNP 执行、讯飞在旁指导——这与国家模型相反，是 RNP 第一次自己训练模型", h=0.5)
band(s, 1.62, 0.50, "服务范围", "3 项行业模型辅助训练服务，每项 1 个边界清晰的固定场景；领域由 RNP 提出、讯飞评估可行性；模型权重由 RNP 训练产生并归 RNP", size=11)
c6 = [(L, 1.85), (2.45, 3.85), (6.40, 3.85), (10.35, 2.48)]
for (cx, cwd), nm, col in zip(c6, ["步骤", "讯飞负责", "RNP 怎么参与", "完成依据"], [MUTE, RED, RNP_C, MUTE]):
    text(s, cx + 0.18, 2.22, min(cwd, 2.2), 0.28, nm, size=11.5, color=col, bold=True, valign="m", margin=(0, 0, 0, 0), sa=0)
rows6 = [
    ("1  确定场景", "评估领域可行性并出技术方案；核实基座版本、规模、许可与工具链",
     "提出领域与场景，明确用户、输入输出和边界", "逐项签署《DLM 场景及技术方案确认书》"),
    ("2  准备数据与路线", "给出训练路线建议（LoRA-SFT、SFT、GRPO 等），每项确认 1 条主要路线",
     "提供合法、足量、有代表性的训练、验证、安全与独立评测数据及环境", "数据与训练路线双方书面确认"),
    ("3  训练与评测", "按约定工作量提供参数建议、查看训练日志、协助分析问题",
     "在模型训练中心执行训练与评测", "指导记录"),
    ("4  服务验收", "基于 RNP 的实际结果完成分析报告",
     "核对指导记录，完成业务验收", "服务验收；模型效果目标与讯飞服务验收分开"),
]
for i, (st, xf6, rn6, dn6) in enumerate(rows6):
    ry = 2.56 + i * 0.78
    card(s, c6[0][0], ry, c6[0][1], 0.72, fill=PANEL2, line=LINE)
    text(s, c6[0][0] + 0.16, ry, c6[0][1] - 0.3, 0.72, st, size=11, color=TXT, bold=True, valign="m", margin=(0, 0, 0, 0), sa=0)
    card(s, c6[1][0], ry, c6[1][1], 0.72, fill=XF)
    body(s, c6[1][0] + 0.16, ry + 0.05, c6[1][1] - 0.32, 0.62, xf6, size=10, valign="m", ls=1.25)
    card(s, c6[2][0], ry, c6[2][1], 0.72, fill=RN)
    body(s, c6[2][0] + 0.16, ry + 0.05, c6[2][1] - 0.32, 0.62, rn6, size=10, valign="m", ls=1.25)
    card(s, c6[3][0], ry, c6[3][1], 0.72, fill=WHITE, line=LINE)
    body(s, c6[3][0] + 0.16, ry + 0.05, c6[3][1] - 0.32, 0.62, dn6, size=10, color=MUTE, valign="m", ls=1.25)
note(s, 5.74, "生产部署、API 开发与应用集成不在本服务内，可另行约定；模型效果目标由 RNP 的训练结果决定，与讯飞的服务验收分开。", h=0.30, size=9.5)
means(s, 6.12, "这是 RNP 首次独立训练模型，需要自己的算法工程师和领域专家；跟岗和指导实操正是为此设计", h=0.6)
