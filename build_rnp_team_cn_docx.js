// 回应 Iara 9/13 WhatsApp（RNP 侧需要什么技能的人、需要多少人）—— 中文工作稿 docx  v0.6
//   在总部研发 v0.4（26 个岗位、111 人）基础上，加入"核心团队 / 扩展团队"两层逻辑（Equipe núcleo / Equipe complementar）：
//     核心团队 43 人 = RNP 编制，接知识转移、项目后常设；扩展团队 68 人 = bolsistas、contratados、合作机构，按工作包进出
//   第一部分：总览表（编号 / 分组 / 岗位 / 核心 / 扩展 / 岗位一句话）
//   第二部分：核心团队与扩展团队（划分依据、经费分类；由谁管理写在各分组说明里）
//   第三部分：逐岗位画像（人员要求 ≤3 条 + 与讯飞协作 ≤2 条），措辞沿用研发 v0.4
//   node build_rnp_team_cn_docx.js  ->  RNP侧团队岗位画像与人数建议-中文稿-v0.6-20260915.docx
const fs = require("fs");
...
// =====================================================================================
const GROUPS = [
  { name: "项目与产品", intro: "负责项目在 RNP 侧的组织、进度、资源与验收，以及国民 AI 应用的产品定义。第一批到位。扩展团队的产品经理与 PMO 由项目经理管理。", roles: [
  ...
  { name: "语料与数据资源", intro: "组织葡语语料的获取、授权、生产与质检，是第一批到位的主力。扩展团队的数据资源、数据工程师和标注员由核心的数据资源与数据质检工程师管理。", roles: [
  ...
  { name: "葡语语言专家", intro: "定义巴西葡语的质量、标注与语音标准。核心 1 人为组长，管理来自高校的 5 名专家。", roles: [
  ...
  { name: "合规与审核", intro: "负责内容审核、数据及模型应用相关的法律合规，验收后承接内容安全审核平台；安全设备运行与技术告警由安全运维岗位负责。扩展团队的审核专家由审核负责人管理。", roles: [
  ...
  { name: "业务与领域专家", intro: "为三个领域大模型确定业务场景并评估领域结果。第二批到位。核心的 3 名业务需求专家各负责一个领域，管理该领域的外部专家。", roles: [
  ...
  { name: "模型与测试工程", intro: "在模型训练中心参与国家模型训练与评测的技术交流和约定实操，并执行 RNP 负责的 DLM 训练与评测；不改变国家模型训练的合同责任分工。第二批到位，是知识转移的核心受训对象。扩展团队的语音算法、评测与测试人员由核心的算法与评测工程师管理。", roles: [
  ...
  { name: "应用与运营", intro: "承担国民 AI 应用与 RNP 系统的集成、用户审批、内容与日常运营；代码维护限于明确交付并授权修改的部分。第三批到位。扩展团队的开发与运营人员由核心的开发骨干、架构师和运营工程师管理。", roles: [
  ...
  { name: "平台运维", intro: "验收后运行国家 AI 能力中心平台。第三批到位，全部为核心团队。华为数据中心层面的运维培训人员不在此列。", roles: [
...
D.push(new Paragraph({ spacing: { after: 320 }, children: [R("回应 Iara 2026 年 9 月 13 日 WhatsApp 请求　·　岗位与人数依据总部研发 v0.4　·　v0.6 · 2026 年 9 月 15 日", { size: 18, color: GRAY })] }));
P(`本稿回答 Iara 的两问：RNP 侧需要什么样的人，需要多少人。团队分两层。**核心团队（Equipe núcleo）${CORE} 人**是 RNP 的自有人员，承接知识转移，项目结束后原地转为常设运营团队；**扩展团队（Equipe complementar）${EXT} 人**由科研津贴人员、合同人员和合作机构人员组成，按工作包投入和退出，由核心团队管理。两层合计 ${TOTAL} 人，${GROUPS.length} 个分组、${NROLES} 个岗位画像。`);
...
// =====================================================================================
H1("二、核心团队与扩展团队");
P(`**核心团队 ${CORE} 人**只放三类人：对外签字负责的，项目结束后要自己扛的，知识转移要一对一带的技术骨干。其中基础设施、法务、安全运维约 6 人可由 RNP 现有人员承担，需新增约 ${CORE - 6} 人。知识转移只覆盖核心团队，再由核心团队里的组长向扩展团队传授。`);
P(`**扩展团队 ${EXT} 人**按工作包配置：语料生产约 30 人，集中在第一批，语料完成即退出；领域与语言专家来自高校和合作机构，按阶段参与；应用开发与运营人员集中在第三批。每一群扩展团队人员都由所在分组的核心岗位管理，写在各分组的说明里。`);
P("按巴西项目经费对人员的分类，核心团队是 RNP 自有人员（CLT），扩展团队对应 bolsistas、contratados 和合作机构人员，分别走 bolsas 与 serviços de terceiros 两条预算线，具体以 RNP 与 MCTI 的经费规则为准。", { after: 60 });

// =====================================================================================
// 三、岗位画像
// =====================================================================================
H1("三、岗位画像");
P("每个岗位两行：**人员要求**说明要什么样的人；**与讯飞协作**说明这个岗位对口讯飞哪支团队、以什么方式一起工作。编号与总览表一致，标题右侧为核心与扩展人数。", { after: 60 });
...
[
  ...
  `2. 分组、岗位、人数与措辞沿用总部研发 v0.4（26 个岗位、合计 111，三批 53 / 25 / 33）。本稿新增核心团队与扩展团队的划分：核心 ${CORE}（含现有人员约 6），扩展 ${EXT}。对外只讲"核心 ${CORE} 加扩展 ${EXT}"，不用"峰值"一词。PPT 第 13 页的 30 / 20 / 30 仍须同步更新。`,
  "3. 相对 v0.4 的四处调整，均对应 RNP 会上原话：审核专家核心 1 人（其余 2 人为扩展）；业务需求专家 3 人全部核心、每领域 1 人（副手要求 DLM 早启动、三领域与 Serpro 等机构协同）；项目经理 1 人兼知识转移与培训协调（Leandro 全程关注培训路径与 ESR）；NLP/DLM 核心 6 人恢复\"能修改训练脚本、适配开源模型\"一条（技术负责人 02:28 原话），并注明 1 至 2 人可赴华参加 CPT（02:30）。",
  "4. 需研发确认三点：扩展团队人员使用源码与评测集的范围及保密约束（对应 v0.4 在 6.4、7.1 加的边界）；知识转移只覆盖核心 43 人；移交后常设团队规模对外如何表述（会上口头为 15 至 20 人，本稿核心团队含运营在内约 25 人）。",
  "5. 英文与葡语版术语：核心团队 Core team / Equipe núcleo；扩展团队 Extended team / Equipe complementar；岗位画像 professional profile / perfil profissional（Marcelo 会上用词）；人数 headcount / quantitativo。不出现 outsourcing / terceirização。",
...
const doc = new Document({
  ...
  sections: [{
    ...
    footers: { default: new Footer({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [
      R("RNP 侧项目团队：核心团队与岗位画像 · 中文工作稿 v0.6 · 第 ", { size: 16, color: GRAY }),
      new TextRun({ children: [PageNumber.CURRENT], font: F, size: 16, color: GRAY }), R(" 页", { size: 16, color: GRAY }) ] })] }) },
    children: D,
  }],
});
const OUT = "RNP侧团队岗位画像与人数建议-中文稿-v0.6-20260915.docx";
