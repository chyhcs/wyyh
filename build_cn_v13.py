# -*- coding: utf-8 -*-
"""
讯飞 × RNP 技术交流 PPT v13：11 页主汇报 + 问答目录页 + 13 页附录，基于华为简报模板。
相对 v11（对齐华为 × 讯飞联合人才培养方案 RNP_AI_Talent_20260909）：
  ...
  · v13：第 4 页去掉与流程卡重复的底部两框，改为「细流程条 + 两个大的讯飞/RNP 栏」，字号放大、块数由 6 减到 4。
...
用法：python3 build_cn_v13.py <template.pptx> <out.pptx>
"""
import sys
...
OUT = sys.argv[2] if len(sys.argv) > 2 else "RNP-讯飞交流-v13.pptx"
RNP_C = "1F4E79"
...
thesis(s, "文本大模型（≥290B 参数）、语音识别、语音合成各 1 个；权重、版本清单、模型卡、评测报告和部署说明交付 RNP", h=0.58)
legend(s, 1.68)
# 细流程条：四步一行，不写说明，说明放到下面两栏
flow = [("语料与基座准备", BOTH), ("训练国家模型", XF), ("评测", BOTH), ("本地交付", XF)]
fw_ = (CW - 3 * 0.22) / 4
for i, (nm, fl) in enumerate(flow):
    fx = L + i * (fw_ + 0.22)
    card(s, fx, 2.06, fw_, 0.62, fill=fl)
    text(s, fx, 2.06, fw_, 0.62, f"{i + 1}  {nm}", size=12.5, color=TXT, bold=True,
         align="c", valign="m", margin=(0, 0, 0, 0), sa=0)
    if i < 3:
        arrow(s, fx + fw_ + 0.005, 2.26, fill=LINE)
text(s, L, 2.80, CW, 0.28,
     "第 1 步两条线同时进行：讯飞在中国境内先行增强基座，RNP 在巴西建设本地语料；本地语料就绪、算力到位后进入第 2 步",
     size=10.5, color=MUTE, valign="m", margin=(0, 0, 0, 0), sa=0)
# 两栏：既是分工，也是各步骤的说明，不再另起底部两框
half_ = (CW - 0.3) / 2
for i, (nm, col, fl, items) in enumerate([
    ("讯飞负责", RED, XF, [
        "**在中国境内先行**：投入自有算力采集公开葡语语料，继续训练基础模型提升通用葡语能力；采集到的公开语料一并提交巴西方",
        "本地语料与算力到位后训练国家模型，出评测报告；在巴西环境部署并交付权重",
        "交付后 12 个月持续优化：文本模型升级 3 次、语音各 1 次",
    ]),
    ("RNP 怎么参与", RNP_C, RN, [
        "**在巴西同步**生产葡语语料并取得授权，约 500 亿 Token",
        "{{协调数据中心建成前用于语料采集与处理的算力}}",
        "专家参与评测评审，与讯飞共同确定测试集与通过规则；组织验收与发布",
    ]),
]):
    cx = L + i * (half_ + 0.3)
    card(s, cx, 3.20, half_, 2.24, fill=fl)
    label(s, cx + 0.28, 3.30, 3.2, nm, color=col, size=13.5)
    body(s, cx + 0.28, 3.76, half_ - 0.56, 1.60, items, size=12, bullet=True, bcolor=col, sa=7, ls=1.3)
means(s, 5.80, "基座增强先行，不占用巴西算力，也不必等本地语料齐备才开始；本地语料和算力到位即可进入国家模型训练，工期风险更小", h=0.62)
