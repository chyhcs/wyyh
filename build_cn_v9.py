# -*- coding: utf-8 -*-
"""
讯飞 × RNP 技术交流 PPT v9：11 页主汇报 + 问答目录页 + 12 页附录，基于华为简报模板。
知识转移课时与人数按赵祥反馈（单轮）刷新：奠基 60 课时/10 天/50 人，跟岗 360 课时/45 天/20 人，
指导实操 20 课时/3 天/20 人，正式移交 168 课时/21 天/40 人；合计 608 课时、约 79 个工作日。
...
用法：python3 build_cn_v9.py <template.pptx> <out.pptx>
"""
import sys
...
OUT = sys.argv[2] if len(sys.argv) > 2 else "RNP-讯飞交流-v9.pptx"
RNP_C = "1F4E79"
...
body(s, L + 0.25, 4.62, CW - 0.5, 0.58, "**知识转移**　四阶段共 608 课时、约 79 个工作日，RNP 核心团队从跟岗到独立操作　　**交付后支持**　现场陪伴 12 个月 + 远程支持 60 个月", size=12, valign="m")
body(s, L, 5.4, CW, 0.7, [
...
means(s, 5.15, "首批组建数据团队与葡语专家，其次配置算法工程师，应用运营团队第三批到位；知识转移的核心学员从三批人员中指定：奠基 50 人、实操 20 人、移交 40 人，名额另计", h=0.7)

# ============================== 9 知识转移：四个阶段 ==============================
s = slide("知识转移的四个阶段：608 课时、约 79 个工作日", size=21, ref="A8")
thesis(s, "一支核心团队，移交后能独立完成约定的平台运维、训练提交、模型部署与发布；50 人建立基础，20 人深入到实操，40 人承接运维移交")
legend(s, 1.78)
phases = [
    ("奠基", "60", "约 10 个工作日 · 50 人",
     "大模型基础、项目架构、语料到服务全流程、安全与运维基础", XF),
    ("跟岗", "360", "约 45 个工作日 · 20 人",
     "讯飞操作，RNP 观察协助；覆盖国家模型训练、12 个月内的模型升级与 3 项 DLM 训练等真实任务", XF),
    ("指导实操", "20", "约 3 个工作日 · 20 人",
     "RNP 操作：数据处理、训练提交、模型部署、监控与故障处理；双方签实操记录", RN),
    ("正式移交", "168", "约 21 个工作日 · 40 人",
...
means(s, 6.05, "79 个工作日是培训活动天数，分散在实施期内，不是连续工期；跟岗随真实训练任务安排")
...
band(s, 5.2, 0.52, "知识转移", "四阶段共 608 课时、约 79 个工作日：奠基 60、跟岗 360、指导实操 20、正式移交 168 课时；核心学员 50 / 20 / 20 / 40 人", fill=REDT, line=None, size=11)
...
rows = [
    ...
    ["合计", "课时跨主题只计一次；阶段日期按批准的知识转移计划", "", "608 课时\n约 79 个工作日", "50/20/20/40", ""],
]
...
note(s, 5.4, ["课时与人数为单轮口径；79 个工作日是培训活动天数之和，不是连续工期；百分比是合格核心学员的培养目标，不是验收阈值；正式移交至少在一年陪伴运维结束前 3 个月启动，不等于 168 课时从该时点开始。",
              "各阶段学员不要求为同一批人，由 RNP 指定稳定且具备基础的学员；正式移交面向运维团队，人数相应放宽；口径：SOW 与批准的知识转移计划。"], h=0.75, size=9.5)
...
text(s, L, 6.0, CW, 0.45, "以上与基础知识转移（四阶段 608 课时）分开，不在当前 SOW 与报价范围内；范围、课时、人数、形式与费用另行约定。口径：方案与建议，非承诺。",
