# -*- coding: utf-8 -*-
"""
讯飞 × RNP 技术交流 PPT v11：11 页主汇报 + 问答目录页 + 12 页附录，基于华为简报模板。
...
用法：python3 build_cn_v11.py <template.pptx> <out.pptx>
"""
import sys
...
OUT = sys.argv[2] if len(sys.argv) > 2 else "RNP-讯飞交流-v11.pptx"
RNP_C = "1F4E79"
...
APPX_CODES = ["A0", "A1", "A2", "A3", "A4", "A5", "A6", "A7", "A8", "A9", "A10", "A11"]
...
legend(s, 1.72)
# 第 1 步是两条线并行，因此第一列做成上下两块；后三步单块，底边对齐
c1w, cw2, g = 4.30, 2.42, 0.25
x1 = L
x2 = x1 + c1w + g
xs_ = [x2 + i * (cw2 + g) for i in range(3)]
top, bot = 2.02, 4.22
text(s, x1, top, c1w, 0.32, "1  语料与基座准备 · 两条线同时展开", size=13, color=TXT, bold=True, valign="m", margin=(0, 0, 0, 0), sa=0)
lanes = [
    ("讯飞 · 在中国境内先行", RED, XF,
     "投入自有算力采集公开葡语语料，继续训练讯飞基础模型，提升其通用葡语能力；采集到的公开语料一并提交巴西方"),
    ("RNP · 在巴西同步", RNP_C, RN,
     "生产葡语语料并取得授权，约 500 亿 Token；讯飞提供规范、工具、试批和抽样质检"),
]
lh = (bot - 2.40 - 0.06) / 2
for i, (nm, col, fl, desc) in enumerate(lanes):
    ly = 2.40 + i * (lh + 0.06)
    card(s, x1, ly, c1w, lh, fill=fl)
    label(s, x1 + 0.22, ly + 0.03, 3.4, nm, color=col, size=11.5)
    body(s, x1 + 0.22, ly + 0.37, c1w - 0.44, lh - 0.42, desc, size=10.5, ls=1.25)
rest = [
    ("训练国家模型", "本地语料就绪、算力到位后，在已具备通用葡语能力的基础模型上继续训练，产出巴西葡语国家模型", XF),
    ("评测", "测试集与通过规则双方确定；讯飞出评测报告，RNP 专家评审", BOTH),
    ("本地交付", "部署在巴西环境，交付权重；12 个月内文本模型升级 3 次、语音各 1 次", XF),
]
for i, (nm, desc, fl) in enumerate(rest):
    bx = xs_[i]
    card(s, bx, top, cw2, bot - top, fill=fl)
    text(s, bx + 0.18, top + 0.12, cw2 - 0.36, 0.38, f"{i + 2}  {nm}", size=13, color=TXT, bold=True, valign="m", margin=(0, 0, 0, 0), sa=0)
    body(s, bx + 0.18, top + 0.58, cw2 - 0.36, bot - top - 0.7, desc, size=10.5, ls=1.3)
arrow(s, x1 + c1w + 0.015, 3.20, fill=LINE)
for i in range(2):
    arrow(s, xs_[i] + cw2 + 0.015, 2.98, fill=LINE)
two_boxes(s, 4.44, 1.40,
          ["先行采集公开葡语语料、增强基础模型的通用葡语能力，使用讯飞在中国境内的自有算力",
           "训练、评测与交付；交付后 12 个月持续优化"],
          ["生产本地葡语语料并取得授权，约 500 亿 Token",
           "专家参与评测评审；组织验收与发布"], size=11.5)
means(s, 5.98, "基座增强先行，不占用巴西算力，也不必等本地语料齐备才开始；本地语料和算力到位即可进入国家模型训练，工期风险更小")
...
# ============================== 9 知识转移：四个阶段 ==============================
s = slide("知识转移：两个轮次，合计 1216 课时、约 158 个工作日", size=21, ref="A8")
...
means(s, 6.05, "卡片为单轮数字，两轮合计 1216 课时、约 158 个工作日；工作日是培训活动天数，分散在实施期内，不是连续工期")
...
text(s, L, 1.8, CW, 0.28, "附录内容除在“项目”中注明“方案”外，均为已写入工作范围（SOW）的口径；{{蓝字}} = 实施前双方共同确定的待确认项。每页底部导航条可在 12 页附录之间直接互跳", size=9.5, color=MUTE, valign="m", margin=(0, 0, 0, 0), sa=0)
menu = [
...
text(s, L, 1.02, CW, 0.5, "项目内的基线支持是文档开放与技术问题解答；以下为可选的主动式高端转移，面向 RNP 核心研发人员与高校课题组，内容可从架构与原理讲到工程经验与源码实践",
...
forms = [
    ...
    ("访问学者计划", "{{拟议：以认知智能全国重点实验室名义邀请 5 到 10 名 RNP 研究人员或在读博士到讯飞，参与多语种模型研发，一期约半年；讯飞不收取费用，人选需面试，差旅自理；可计入 RNP 人才培养计划}}"),
]
fw = (CW - 2 * 0.3) / 3
fy, fh = 4.36, 1.45
