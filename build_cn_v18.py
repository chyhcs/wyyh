# -*- coding: utf-8 -*-
"""
讯飞 × RNP 技术交流 PPT v18：13 页主汇报 + 问答目录页 + 13 页附录，基于华为简报模板。
...
用法：python3 build_cn_v18.py <template.pptx> <out.pptx>
"""
import sys
...
OUT = sys.argv[2] if len(sys.argv) > 2 else "RNP-讯飞交流-v18.pptx"
RNP_C = "1F4E79"
...
def appx_nav(s, cur):
    ...
    for code in APPX_CODES:
        ...
        x += cw_ + gap
    b = chip(s, 8.05, y, 1.35, h, "返回目录", fill=WHITE, color=RNP_C, size=9.5, line=LINE)
    BACK.append(b)
    tgt = MAIN_OF.get(cur)
    if tgt is not None:
        c = chip(s, 9.50, y, 1.35, h, "返回第 %d 页" % PAGENO[id(tgt)], fill=WHITE, color=RED, size=9.5, line=LINE)
        BACK2.append((c, tgt))
...
funcs = ["问答", "检索", "文档分析", "研究辅助", "语音输入与朗读", "笔记与写作"]
